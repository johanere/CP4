/*
   ...
   Run as: mpirun -n <#threads> ./Ising.x  <filename>
*/
#include "mpi.h"
#include <cmath>
#include <iostream>
...
int main(int argc, char* argv[])
{
  char *outfilename;
  long idum;
  int **spin_matrix, n_spins, mcs, my_rank, numprocs,equilibrium_treshold;
  ...
  n_spins = 80; mcs = 1100000;  equilibrium_treshold=1000000; initial_temp = 2.24; final_temp = 2.30; temp_step =0.005;
  /*
  ...
  */
  ...
  for ( double temperature = initial_temp; temperature <= final_temp; temperature+=temp_step){
    if (my_rank == 0){cout<<temperature<<endl;}
    ...
    for (int cycles = myloop_begin; cycles <= myloop_end; cycles++){
      ...
      //____________discard measurements before equilibrium treshold is obtained
      if(cycles>=equilibrium_treshold)
      {
    ...
    // print results
    if ( my_rank == 0) {
      output(n_spins, mcs-equilibrium_treshold, temperature, total_average);
    }
  }
...
void output(int n_spins, int effective_cycles, double temperature, double *total_average)
{
  double norm = 1/((double) (effective_cycles));  // divided by total number of cycles
