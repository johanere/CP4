...
for j in range(len(outdata)):
    ...
    #plot
    plt.plot(mcs[30:],E[30:],label="%s"%(lab[j]))       
print len(E)
print E[100]
N=float(hold[0])
pow=int(np.log10(N))
...
for j in range(cutoff,int(len(E))):
    err[j]=abs((E[j]-analytical)/analytical)
...
plt.grid('on')
plt.legend(loc=0)
plt.title('$<E>$ for $L=%s$, $T=%s$ over $4 \cdot 10^%s$ Monte Carlo cycles'%(L,T[1],pow), fontsize=14)
plt.show()       
