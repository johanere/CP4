/*
   ...
   Compile with: mpic++ -O3 -o  Ising.x MPIising.cpp
   Run as: mpirun -n <#threads> ./Ising.x  <filename> <#spins> <#mcs> <t_0> <t_f> <t_steps>
*/
#include "mpi.h"
#include <cmath>
#include <iostream>
...
int main(int argc, char* argv[])
{
  char *outfilename;
  long idum;
  int **spin_matrix, n_spins, mcs, my_rank, numprocs, ptotal;
  double w[17], average[6], total_average[6],
         initial_temp, E, M,C;
  ...
  */
  ...
  MPI_Bcast (&ptotal, 1, MPI_INT, 0, MPI_COMM_WORLD);
  ...
  idum = -1-my_rank;  // random starting pointaaa
  // Start Monte Carlo sampling by looping over T first
  double  TimeStart, TimeEnd, TotalTime;
  TimeStart = MPI_Wtime();

  //redefining mcs to mcs per print cycle
  mcs=mcs/ptotal;
  ...
  //    initialise energy, magnetization, and number of configuartions
  E = M = C= 0.;
  // initialise array for expectation values
  initialize(n_spins, spin_matrix, E, M);
  // setup array for possible energy changes
  for( int de =-8; de <= 8; de++) w[de+8] = 0;
  for( int de =-8; de <= 8; de+=4) w[de+8] = exp(-de/initial_temp);
  for( int i = 0; i < 6; i++) average[i] = 0.;
  for( int i = 0; i < 6; i++) total_average[i] = 0.;
  double totalC=0.;
//initializing print cycles
  for ( double pcycle = 1; pcycle <= ptotal; pcycle++)
  {
    // start Monte Carlo computation
    {
      for (int cycles = myloop_begin; cycles <= myloop_end; cycles++)
      {
        Metropolis(n_spins, idum, spin_matrix, E, M, w,C);
      ...
      // print results
      if ( my_rank == 0) {
        output(pcycle*mcs,n_spins, mcs*pcycle, initial_temp, total_average);
      }
    }
  }
...
void Metropolis(int n_spins, long& idum, int **spin_matrix, double& E, double&M, double *w,double&C)
{
  // loop over all spins
  for(int y =0; y < n_spins; y++) {
    for (int x= 0; x < n_spins; x++){
...
void output(int MCc,int n_spins, int mcs, double temperature, double *total_average)
{
  ...
  ofile << setw(15) << setprecision(8) << MCc; //current number of MCcycles
