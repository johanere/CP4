...
Run as
./executable Outputfile numberof spins number of MC cycles initial temp final temp tempstep <total prints to file> <init_config>(0=random, all other=ordered up)
...
void MetropolisSampling(int NSpins, int MCcycles, double Temperature, vec &ExpectationValues,int ptotal,int intital_config)
{
  ...
  int MCs_per_print_cycle=MCcycles/ptotal; //_____
  int cycles=0; //_____
  for (int pcycles=1; pcycles<=ptotal; pcycles++)
  {
    for (;cycles < pcycles*MCs_per_print_cycle; cycles++)
    {
      ...
      output(NSpins, cycles, Temperature, ExpectationValues);
  }
} // end of Metropolis sampling over spins
void output(int NSpins, int cycles, double temperature, vec ExpectationValues)
{
